...
- **Deployment-Ready:** Fully configured with a unified `app.py`, `requirements.txt`, and a `Dockerfile` specifically optimized for **Hugging Face Spaces**.
...
- **`/pre_match`**: Expects `team_id`, `num_matches`, and `opponent_id`. Returns Starting XI, strategy, and training plan.
- **`/post_match`**: Expects `team_id` and `event_id`. Returns match and fatigue analysis.
